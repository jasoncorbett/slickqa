    ...
    onReady: function() {
        ...
        _.each(this.data.proj.releases, function(release) {
            ...
            if(this.data.proj.defaultRelease == release.id) {
                this.releases[this.releases.length] = ['<a href="#/testmgmt/addbuild/' + release.id + '">' + release.name + '</a>', new Date(release.target), '<a href="javascript:;" data-release-id="' + release.id + '" data-release-name="' + release.name + '" id="defaultRelease" class="defaultRelease">default</a>'];
            } else {
                this.releases[this.releases.length] = ['<a href="#/testmgmt/addbuild/' + release.id + '">' + release.name + '</a>', new Date(release.target), '<a href="javascript:;" data-release-id="' + release.id + '" data-release-name="' + release.name + '" class="notDefaultRelease">make default</a>'];
            }
        }, this);
    },
